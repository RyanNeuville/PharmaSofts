...
Public Class UtilisateursControl
    ...
    Public Sub New()
        ...
        lbl.Text = "Liste des Pharmaciens"
        ...
        AddHandler btnAjouter.Click, AddressOf AjouterPharmacien
        ...
        AddHandler btnModifier.Click, AddressOf ModifierPharmacien
        ...
        AddHandler btnSupprimer.Click, AddressOf SupprimerPharmacien
    ...
    Private Sub ChargerPharmaciens()
        dgv.Rows.Clear()
        Try
            Using conn = GetConnection()
                Dim cmd As New MySqlCommand("SELECT id, nom, email, telephone FROM pharmaciens", conn)
    ...
    Private Sub AjouterPharmacien(sender As Object, e As EventArgs)
        Dim frm As New FormPharmacien()
        frm.ShowDialog()
        ChargerPharmaciens()
    End Sub

    Private Sub ModifierPharmacien(sender As Object, e As EventArgs)
        If dgv.SelectedRows.Count = 0 Then
            MessageBox.Show("Veuillez sélectionner un pharmacien.", "Attention", MessageBoxButtons.OK, MessageBoxIcon.Warning)
            Return
        End If
        ...
        Dim frm As New FormPharmacien(id, nom, email, tel)
        frm.ShowDialog()
        ChargerPharmaciens()
    End Sub

    Private Sub SupprimerPharmacien(sender As Object, e As EventArgs)
        If dgv.SelectedRows.Count = 0 Then
            MessageBox.Show("Veuillez sélectionner un pharmacien.", "Attention", MessageBoxButtons.OK, MessageBoxIcon.Warning)
            Return
        End If

        If MessageBox.Show("Voulez-vous vraiment supprimer ce pharmacien ?", "Confirmation", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = DialogResult.Yes Then
            Try
                Dim id As Integer = CInt(dgv.SelectedRows(0).Cells("ID").Value)
                Using conn = GetConnection()
                    Dim cmd As New MySqlCommand("DELETE FROM pharmaciens WHERE id=@id", conn)
